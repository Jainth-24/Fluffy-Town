...
import { ADD_ADDRESS, REMOVE_ADDRESS, UPDATE_ADDRESS, UPDATE_DEFAULT_ADDRESS_MUTATION } from './mutations/address';
...
export async function getProduct(handle: string, selectedOptions: any[]): Promise<Product | any | undefined> {
...
export async function getProductRecommendations(productId: string): Promise<Product[] | any> {
  ...
  const originalProduct = mergedProducts.map((item: any) => item.id).indexOf(productId);
...
export async function deleteAddress({ variables }: { variables: { customerAccessToken: string; id: string } }) {
  const data = await shopifyFetch<{
    data: {
