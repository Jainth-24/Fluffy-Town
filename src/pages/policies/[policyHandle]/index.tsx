...
const Policy: React.FC<PolicyProps> = ({ policy }) => {
  ...
  return (
    <StoreLayout>
      <Section display="flex" className="flex-col items-baseline w-full gap-8 md:flex-row justify-center">
        <PageHeader heading={policy.title} className=" md:sticky top-48 md:w-5/12 text-5xl">
          <Button onClick={() => router.push('/')} color="warning" className="">
            &larr; Back to Home
          </Button>
        </PageHeader>
