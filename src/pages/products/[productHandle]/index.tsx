    ...
    <StoreLayout>
      ...
      <Section className="px-0 md:px-8 lg:px-12">
        <div className="grid items-start md:grid-cols-2 md:gap-6 lg:grid-cols-3 lg:gap-20">
          <ProductGallery media={product?.media.nodes} className="w-full lg:col-span-2" />
            ...
            <section className="flex w-full max-w-xl flex-col gap-8 p-6 md:mx-auto md:max-w-sm md:px-0">
              ...
              <div className="grid gap-4 py-4">
                {shop?.shippingPolicy?.body && (
                  <ProductDetail
                    title="Shipping"
                    content={getExcerpt(shop?.shippingPolicy.body)}
                    learnMore={`/policies/${shop?.shippingPolicy.handle}`}
                  />
                )}
                {shop?.refundPolicy?.body && (
                  <ProductDetail
                    title="Returns"
                    content={getExcerpt(shop?.refundPolicy.body)}
                    learnMore={`/policies/${shop?.refundPolicy.handle}`}
                  />
                )}
              </div>
            </section>
